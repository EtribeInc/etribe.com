...
UIOverlayBinder.prototype.bindMediaButtons = function () {
    ...
    // catch up on roster changes that happened before the HTML was mounted
    if (Networking.instance && Networking.instance.players) {
        this.populateUsers(Networking.instance.players, Networking.instance.admin);
    }
...
UIOverlayBinder.prototype.populateUsers = function (users, admin) {
    ...
    // the P2P room connects before the overlay HTML is fetched - re-populated
    // from bindMediaButtons once the template exists
    ...
    for (const [key, value] of userArray) {
        ...
        // textContent, not innerHTML: username is peer-supplied
        userEntry.querySelector("p").textContent = value.username;
...
UIOverlayBinder.prototype.showRoomCode = function () {
    if (!window.TellyP2P || !TellyP2P.code) return;
    var self = this;
    var chip = document.getElementById("tc-room-code-chip");
    if (chip) chip.remove();
    chip = document.createElement("div");
    chip.id = "tc-room-code-chip";
    chip.innerHTML = "Room <b>" + TellyP2P.code + "</b>";
    chip.title = "Click to copy the invite link";
    chip.style.cssText = "position:fixed;top:12px;right:12px;z-index:1000;cursor:pointer;" +
        "background:rgba(26,26,31,0.8);color:#fff;padding:6px 12px;border-radius:16px;" +
        "font-size:13px;letter-spacing:1px;user-select:none;";
    chip.addEventListener("click", () => self.copyShareLink());
    document.body.appendChild(chip);
    this.on("destroy", () => chip.remove());
};
