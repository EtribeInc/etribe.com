...
Networking.prototype.initialize = function () {
    ...
    // ?name= param / airlock input (persisted in localStorage), with a
    // typeof guard in case a stale constants.js is deployed
    var storedName = typeof getStoredUsername === 'function' ? getStoredUsername() : '';
    this.username = storedName || ('Guest ' + Math.floor(1000 + Math.random() * 9000));
...
Networking.prototype.onHello = function (data, peerId) {
    ...
    // Peer-supplied name ends up in DOM innerHTML (attendees list, share
    // dialog) - never trust it raw.
    data.name = (typeof sanitizeUsername === 'function' ?
        sanitizeUsername(data.name) : String(data.name || '').slice(0, 16)) || 'Guest';
